document.addEventListener(
	...
	function() {
		...
		const trackerAppActionsContainerEl = document.querySelector(".tracker-content .app__body--actions");
		...
		function initCoronaTracker() {
			chrome.storage.local.get(["searchedLocation"], storage => {
				...
				while (trackerAppActionsContainerEl.firstChild.tagName != "BUTTON") {
					trackerAppActionsContainerEl.removeChild(trackerAppActionsContainerEl.firstChild);
				}
		...
		refreshButton.addEventListener("click", () => {
			...
			} else if (searchedLocation.toLowerCase() != locationInputEl.value.toLowerCase() && isValidLocation) {
		...
		locationInputEl.addEventListener("keydown", e => {
			...
			} else if (e.keyCode == ENTER_KEYCODE && isValidLocation == false && !locationInputEl.value.length == 0) {
				showInfoIfLocationIsNotValid();
		...
		function fetchLocationDetails(location) {
			...
			let url = location.toLowerCase() == "world" ? `${API_URL}/all` : `${API_URL}/countries/${location.toLowerCase()}`;
		...
		function updateApp(data) {
			...
			animateValue(confirmedCasesEl, parseInt(confirmedCasesEl.innerHTML), data.infected, counterStepAnimationDuration);
			animateValue(recoveredEl, parseInt(recoveredEl.innerHTML), data.recovered, counterStepAnimationDuration);
			animateValue(deathsEl, parseInt(deathsEl.innerHTML), data.deaths, counterStepAnimationDuration);
		}
			...
			return fetch(`${API_URL}/all`)
				...
				.then(updatedTime => {
					...
					if (showedLastUpdatedTime == false) {
						showInfoText(trackerAppActionsContainerEl, `Last updated : ${lastUpdated}`, 2500);
		...
		function failedToFetchData() {
			...
			showInfoText(trackerAppActionsContainerEl, "check your connection!", 2000, "rgba(255, 0, 0, 1)");
		...
		function showInfoIfLocationIsNotValid() {
			...
			showInfoText(trackerAppActionsContainerEl, "Location not found!", 1500, "rgba(255, 0, 0, 1)");
		}
		...
		function showEffectedChange(location) {
			...
			if (location == "world") {
				chrome.storage.local.get(["totalInfected", "totalRecovered", "totalDeaths"], storage => {
					if (storage.totalInfected != 0 && storage.totalRecovered != 0 && storage.totalDeaths != 0) {
			...
			function showChange() {
				setTimeout(() => {
					...
					if (todayCases != 0 && todayCases != undefined) addClassAndChangeHTML(infectedChangeEl, todayCases, "show");
					if (todayRecovered != 0 && todayRecovered != undefined)
						addClassAndChangeHTML(recoveredChangeEl, todayRecovered, "show");
					if (todayDeaths != 0 && todayDeaths != undefined) addClassAndChangeHTML(deathsChangeEl, todayDeaths, "show");
